...
export async function generateAImonMeta(
  promptText,
  { baseImageDataUrl, apiKey } = {}
) {
  const ai = getClient(apiKey);
  if (!ai) throw new Error("GEMINI_API_KEY missing");
  ...
  const parts = [
    {
      text: [
        promptText ||
          "Design a new original battle-creature based on the provided reference.",
...
export async function generateActionImage(
  { baseImageDataUrl, name, type, characteristics, apiKey },
  power
) {
  const ai = getClient(apiKey);
  if (!ai) throw new Error("GEMINI_API_KEY missing");
  ...
  const actionText = [
    `Create a high-quality PNG of the same original battle-creature performing the action "${
      power?.name || "Unknown"
    }".`,
    power?.description ? `Action details: ${power.description}.` : "",
    `Appearance guide — Name: ${name || "Unknown"}, Type(s): ${
      Array.isArray(type) ? type.join("/") : type || "Unknown"
    }. ${characteristics || ""}`,
