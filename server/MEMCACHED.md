...
```javascript
// POST /api/generate
// When new AImon is created, cache is updated (not invalidated)
const cached = await cache.get("gallery:all");
if (cached && Array.isArray(cached)) {
  cached.unshift(saved); // Add new AImon to cache
  await cache.set("gallery:all", cached, 300);
}

// PATCH /api/aimon/:id/like
// When AImon is liked, update it in cache
const cached = await cache.get("gallery:all");
if (cached && Array.isArray(cached)) {
